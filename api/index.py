...
def extract_info(data):
    try:
        ...
        try:
            ...
            lvl = 0
        return {"nickname": str(nick), "level": lvl, "region": str(reg),
                "last_login": str(last), "last_login_raw": raw}
    except Exception:
        return {"nickname": "Unknown", "level": 0, "region": "Unknown",
...
def fetch_info(uid, region="ind"):
    try:
        ...
        if r.status_code != 200:
            return None, f"Info HTTP {r.status_code}"
...
def fetch_ban(uid):
    try:
        ...
        if r.status_code != 200:
            return None, None, f"Ban HTTP {r.status_code}"
...
def bancheck():
    try:
        ...
        if not uid or not uid.isdigit():
            return jsonify({"success": False, "error": "valid uid required", "dev": DEV}), 400

        info, ierr = fetch_info(uid, region)
        is_banned, period, berr = fetch_ban(uid)

        errs = []
        if ierr: errs.append(ierr)
        if berr: errs.append(berr)

        status = "UNKNOWN" if is_banned is None else ("BANNED" if is_banned else "NOT BANNED")

        last_ago = None
        if info and info.get("last_login_raw"):
            last_ago = humanize_ago(info["last_login_raw"])
        ...
        if is_banned and info:
            ...
            result["banned_since_ago"] = last_ago

        return jsonify(result)
    except Exception as e:
        return jsonify({"success": False, "error": str(e)[:100], "dev": DEV}), 500
...
@app.route('/')
def home():
    return jsonify({"name": "Ban Checker API", "dev": DEV})
